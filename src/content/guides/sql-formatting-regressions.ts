...
export const sqlFormattingRegressionsGuide: Guide = {
  ...
  sections: [
    ...
    { heading: "Try it in the tool", body: ["Paste this example into SQL Formatter. Choose a dialect, format, and compare the quoted values and the comment boundary before using the result.", { code: "SELECT 'a  b', 'SELECT, FROM' FROM users -- keep comment\nWHERE id = 1;", label: "SQL example" }, "Repeat with your own non-sensitive examples. A clean layout does not tell you whether a table exists or whether a query returns the rows you intended."] },
