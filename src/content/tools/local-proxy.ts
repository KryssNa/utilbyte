...
export const localProxyArticle: ToolArticleContent = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "HTTPS to HTTP. A tunnel may terminate HTTPS before forwarding to a local HTTP service. Configure the application to trust forwarded-protocol headers only from a proxy you control; an arbitrary caller can forge those headers.",
      ...
      bullets: [
        ...
        "Configure trusted-proxy handling before relying on X-Forwarded-Proto.",
  ...
  example: {
    ...
    note: "This is the first thing that happens to almost everyone, and the error is at least self-explanatory - many frameworks are less helpful. The X-Forwarded-Proto header can help construct absolute URLs only when it comes from a proxy you control and your application is configured to trust that proxy.",
  },
  limitations: [
    ...
    "The endpoint, received requests and forwarding state live on a hosted server. Anyone with a bin ID can inspect requests or change forwarding settings; no owner authentication or automatic retention cleanup is implemented.",
    "Only configure a public test destination that you control. This service is not an access-control boundary; do not rely on it to protect private services.",
