...
export const formatConverterArticle: ToolArticleContent = {
  ...
  sections: [
    {
      ...
      bullets: [
        "Transparency: kept by PNG and WebP output. Lost by JPEG.",
    ...
    },
    {
      ...
      body: [
        ...
        "GIF can be opened as an input when the browser supports it, but this tool does not export GIF. Animation is reduced to a still frame during the canvas conversion.",
        "BMP may be readable as an input, but it is not an output option. Choose PNG, JPEG or WebP for a raster download.",
    ...
    },
    {
      ...
      body: [
        ...
        "Encoding has its own limits. The output choices are PNG, JPEG, WebP and an SVG wrapper containing a PNG image. If the browser cannot produce the chosen raster format, the tool reports an error instead of giving a file with a misleading extension.",
      ],
    },
  ],
  example: {
  ...
  limitations: [
    ...
    "Transparency is lost when converting to JPEG and is filled with white. There is no way to recover it afterwards.",
