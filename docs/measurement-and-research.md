...
Compare A (desktop navigator collapsed) and B (expanded contextual navigator) using the same build and tool engines. Use the “Collapse desktop navigator” setting in Tools. At narrow widths both variants use the same drawer; assess it separately. Alternate AB and BA order within each participant group. Focus mode is off for the initial finding tasks.
...
5. Enter the synthetic SQL fixture, open/close Tools, toggle Focus mode, and verify the input survives.
...
- Roll back navigation independently of formatter correctness changes if it causes input loss, inaccessible controls, or persistent editor overflow. Use collapsed navigation as the comparison variant.
